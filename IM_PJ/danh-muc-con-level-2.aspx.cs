...
namespace IM_PJ
{
    public partial class danh_muc_con_level_2 : System.Web.UI.Page
    {
        ...
        public void LoadData()
        {
            ...
            int ParentID = Request.QueryString["pid"].ToInt(0);
            if (ParentID > 0)
            {
                var parent = CategoryController.GetByID(ParentID);
                if (parent != null)
                {
                    if (parent.CategoryLevel == 1)
                    {
                        ...
                        if (parent0 != null)
                            lblCategoryName.Text = parent0.CategoryName + " -> " + parent.CategoryName;
                        ltrAddnew.Text += "<a href=\"/danh-muc-con-level-1?pid=" + parent0.ID + "\" class=\"btn primary-btn h45-btn\">Trở về</a>  ";
                        ltrAddnew.Text += "<a href=\"/them-danh-muc-con-level-2?pid=" + ParentID + "\" class=\"h45-btn btn\" style=\"background-color: #ff3f4c\">Thêm mới danh mục con</a>";

                        if (Request.QueryString["s"] != null)
                        {
                            string s = Request.QueryString["s"];
                            txtAgentName.Text = s;
                            a = CategoryController.GetByParentID(s, ParentID);
                        }
                        else
                        {
                            a = CategoryController.GetByParentID("", ParentID);
                        }
                        if (a.Count > 0)
                        {
                            pagingall(a);
                    ...
                    {
                        Response.Redirect("/danh-muc-con-level-1?pid=" + ParentID + "");
                    }
                }
                else
                {
        ...
        public void pagingall(List<tbl_Category> acs)
        {
            ...
            if (acs.Count > 0)
            {
                ...
                for (int i = FromRow; i < ToRow + 1; i++)
                {
                    ...
                    html.Append("   <td>");
                    html.Append("       <a href=\"/thong-tin-danh-muc-con-level-2.aspx?id=" + item.ID + "\" class=\"btn primary-btn h45-btn\">Chi tiết</a>");
                    html.Append("       <a href=\"/danh-sach-san-pham.aspx?categoryid=" + item.ID + "\" class=\"btn primary-btn h45-btn\">Sản phẩm</a>");
                    //html.Append("       <a href=\"/danh-muc-con-level-2.aspx?pid=" + item.ID + "\" class=\"btn primary-btn h45-btn\">Danh mục con</a>");
                    html.Append("   </td>");
                    html.Append("</tr>");
                }
            }
