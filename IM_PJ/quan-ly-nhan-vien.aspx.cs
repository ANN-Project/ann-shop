    ...
    public partial class quan_ly_nhan_vien : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    int agentID = Request.QueryString["agentid"].ToInt();
        ...
        {
            List<tbl_Account> a = new List<tbl_Account>();
            int agentid = Request.QueryString["agentid"].ToInt(0);
            ltrAddnew.Text = "<a href=\"/them-nhan-vien?agentid=" + agentid + "\" class=\"h45-btn btn\" style=\"background-color: #ff3f4c\">Thêm mới</a>";
            if (Request.QueryString["s"] != null)
            {
                ...
                if (agent != null)
                {
                    lblAgentName.Text = agent.AgentName;
                }
                string username = Session["userLoginSystem"].ToString();
                var acc = AccountController.GetByUsername(username);
                if (acc != null)
                {
                    if (acc.RoleID == 0)
                    {
                        a = AccountController.GetAllByAgentID(s, agentid);
                        ltrback.Text = "<a href=\"/quan-ly-dai-ly.aspx\" class=\"btn primary-btn fw-btn not-fullwidth\">Trở về</a>";

                    }
                    else if (acc.AgentID == agentid && acc.RoleID == 1)
                    {
                        a = AccountController.GetAllByAgentIDNotManager(s, agentid);
                    }
                }
            ...
            else
            {
                ...
                if (acc != null)
                {
                    if (acc.RoleID == 0)
                    {
                        a = AccountController.GetAllByAgentID("", agentid);
                        ltrback.Text = "<a href=\"/quan-ly-dai-ly.aspx\" class=\"btn primary-btn fw-btn not-fullwidth\">Trở về</a>";
                    }
                    else if (acc.AgentID == agentid && acc.RoleID == 1)
                    {
                        a = AccountController.GetAllByAgentIDNotManager("", agentid);
                    }
                }
        ...
        #region Paging
        public void pagingall(List<tbl_Account> acs)
        {
            int PageSize = 15;
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("/quan-ly-nhan-vien?agentid=" + agentid + "&s=" + search + "");
            }
            else
            {
                Response.Redirect("/quan-ly-nhan-vien?agentid=" + agentid + "");
            }
        }
    }
}