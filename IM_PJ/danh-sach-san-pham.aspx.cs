...
namespace IM_PJ
{
    public partial class danh_sach_san_pham : System.Web.UI.Page
    {
        ...
        public void LoadData()
        {
            ...
            List<View_ProductList> a = new List<View_ProductList>();
            int categoryID = Request.QueryString["categoryid"].ToInt(0);
            if (categoryID > 0)
            {
                var cate = CategoryController.GetByID(categoryID);
                if (cate != null)
                {
                    if (cate.CategoryLevel == 0)
                    {
                        ltrBack.Text = "<a href=\"/quan-ly-danh-muc-san-pham\" class=\"btn primary-btn h45-btn\">Trở về</a>";
                    }
                    else if (cate.CategoryLevel == 1)
                    {

                        ltrBack.Text = "<a href=\"/danh-muc-con-level-1?pid=" + cate.ParentID + "\" class=\"btn primary-btn h45-btn\">Trở về</a>";
                    }
                    else if (cate.CategoryLevel == 2)
                    {

                        ltrBack.Text = "<a href=\"/danh-muc-con-level-2?pid=" + cate.ParentID + "\" class=\"btn primary-btn h45-btn\">Trở về</a>";
                    }
                    lblCategoryName.Text = cate.CategoryName;
                    ltrAddnew.Text = "<a href=\"/them-san-pham?categoryid=" + categoryID + "\" class=\"h45-btn btn\" style=\"background-color: #ff3f4c\">Thêm mới</a>";
                }
        ...
        #region Paging
        public void pagingall(List<View_ProductList> acs)
        {
            int PageSize = 15;
            ...
            if (acs.Count > 0)
            {
                ...
                for (int i = FromRow; i < ToRow + 1; i++)
                {
                    ...
                    html.Append("   <td>" + string.Format("{0:N}", item.CostOfGood) + "</td>");
                    html.Append("   <td>" + string.Format("{0:N}", item.Retail_Price) + "</td>");
                    ...
                    html.Append("   <td>");
                    html.Append("       <a href=\"/thong-tin-san-pham.aspx?id=" + item.ID + "\" class=\"btn primary-btn h45-btn\">Chi tiết</a>");
