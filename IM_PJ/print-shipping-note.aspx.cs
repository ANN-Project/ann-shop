        ...
        public void LoadData()
        {
            ...
            if (ID > 0)
            {
                ...
                if (order != null)
                {
                    ...
                    {
                        error += "<p>- Đơn hàng chưa hoàn tất!</p>";
                    }

                    if(order.ShippingType == 1)
                    {
                        error += "<p>- Đơn hàng này đang ở trạng thái <strong>Lấy trực tiếp</strong>. Hãy đổi sang trạng thái khác!</p>";
                    }
                    ...
                    {
                        rowHtml += Environment.NewLine + String.Format("        <p><span class=\"cod\">Thu hộ (COD): {0}</span></p>", string.Format("{0:N0}", Convert.ToDouble(order.TotalPrice)));
                    }
                    else
                    {
                        rowHtml += Environment.NewLine + String.Format("        <p><span class=\"cod\">Thu hộ (COD): KHÔNG</span></p>");
                    }
                    ...
                    {
                        rowHtml += Environment.NewLine + String.Format("        <p class=\"delivery\"><span>Gửi xe</span></p>");
                    }
                    ...
                    {
                        rowHtml += Environment.NewLine + String.Format("        <p><span class=\"cod\">Thu hộ (COD): KHÔNG THU</span></p>");
                    }
                    rowHtml += Environment.NewLine + String.Format("    </div>");
                    ...
                    rowHtml += Environment.NewLine + String.Format("        <p><span>Địa chỉ: <span class=\"address\"> {0}</span></span></p>", order.CustomerAddress);
                    rowHtml += Environment.NewLine + String.Format("    </div>");
                    rowHtml += Environment.NewLine + String.Format("</div>");
